...
/*
 * ssh cr gerrit review $crNumber,$crPatchSet --submit
 */
function stepSubmitCr(arg, cb) {
    assert.object(arg.currentCr, 'arg.currentCr');
    assert.string(arg.branchConfigCr, 'arg.branchConfigCr');
    ...
    common.forkExecWaitAndPrint({
        argv: [
            ...
            '-o', 'UserKnownHostsFile=/dev/null', '-p', '29418',
            arg.gerritUsername + '@cr.joyent.us',
            'gerrit', 'review', '--submit',
            format('%s,%d',
                arg.currentCr.number,
...
/*
 * Submit (i.e. integrate) the given change.
 *
 * XXX Really should ensure don't have updates to make (unless just
 *     commit msg updates, in which case just do them), else error out.
 *
 * Dev Notes:
 * - https://cr.joyent.us/Documentation/rest-api-changes.html#submit-change
 *   has a `"wait_for_merge":true` option that sounds really interesting to
 *   me as I would like to wait for gerrit to push upstream. I'm not sure
 *   if that is what this option means. Also `gerrit review --submit` doesn't
 *   seem (to me) to have this option.
 */
function grrSubmit(opts, cb) {
    assert.object(opts, 'opts');
    assert.object(opts.log, 'opts.log');
    assert.optionalBool(opts.dryRun, 'opts.dryRun');
    assert.func(cb, 'cb');
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        // XXX START HERE:
        //  see if can move ensureIssueId out to a shareable step and used that
        //  to get arg.issueName/issueTitle for:
        function printIssue(arg, next) {
            console.log('Issue: %s %s', arg.issueName, arg.issueTitle);
            next();
        },

        // XXX Confirm the CR is open and has reviews required.
        //      ctx.currentCr.open === true
        //      ctx.currentCr.currentPatchSet.approvals:
        //            "approvals": [
        //              {
        //                "type": "Code-Review",
        //                "description": "Code-Review",
        //                "value": "1",
        //                "grantedOn": 1518822513,
        //                "by": {
        //                  "name": "Marsell Kukuljevic",
        //                  "email": "marsell@joyent.com",
        //                  "username": "marsell"
        //                }
        //              },
        //              {
        ...
        // XXX abort if there are unpushed commits (lastPushedSha)
        //        stepGetBranchConfigLastPushedSha,

        // XXX update commit messages for review lines if needed
        //     or start by just bailing if it changes.

        stepSubmitCr,

        function printResult(arg, next) {
            console.log('Submitted CR %s patchset %s <https://cr.joyent.us/%s>',
                arg.currentCr.number, arg.currentCr.currentPatchSet.number,
                arg.currentCr.number);
            console.log('(You can delete this grr-* branch with `grr -D`.)')
            next();
        }
