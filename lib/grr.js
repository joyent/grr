...
var fsplit = require('./common').fsplit;
...
function stepGetBranchConfigExtraIssues(arg, cb) {
    ...
    getBranchConfigKey('extraIssues', arg, function (err, val) {
        ...
        fsplit(val, ',').forEach(function (id, _) {
            arg.extraIssues.push({ issueId: id });
        });
...
/*
 * Add and remove any extra issues requested, then fill out the rest of the
 * required information for each of them.
 */
function stepProcessExtraIssues(arg, cb) {
    // FIXME assert

    const to_remove = fsplit(arg.removeExtraIssuesArg, ',');

    arg.extraIssues = arg.extraIssues.filter(function (item) {
        return to_remove.indexOf(item.issueId) !== -1;
    });

    const to_add = fsplit(arg.addExtraIssuesArg, ',');

    to_add.forEach(function (item, _) {
	// FIXME: need to transform this into an id like Validate above
        arg.extraIssues.push({ issueId: item });
    });

    arg.extraIssues.forEach(function (issue, _) {
        if (/^\d+$/.test(issue.issueId)) {
            issue.issueType = 'github';
            issue.issueName = arg.repoName + '#' + issue.issueId;
        } else {
    ...
    vasync.forEachPipeline({
        inputs: arg.extraIssues,
        func: function (issue, next) {
            getIssueTitle(arg, issue, function (err, title) {
                if (err) {
                    next(err);
                    return;
                }
...
function stepSetBranchConfigExtraIssues(arg, cb) {
    ...
    arg.branchConfigExtraIssues = arg.extraIssues.map(function (issue) {
...
function grrDelete(opts, cb) {
    ...
    if (opts.addExtraIssuesArg || opts.removeExtraIssuesArg) {
        cb(new VError('extra issues specified when deleting'));
        return;
    }
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        // FIXME: complain about extra args here?
...
function grrUpdateOrCreate(opts, cb) {
    ...
    assert.object(opts.log, 'opts.log');
    assert.optionalString(opts.issueArg, 'opts.issueArg');
    assert.optionalString(opts.addExtraIssuesArg, 'opts.addExtraIssuesArg');
    assert.optionalString(opts.removeExtraIssuesArg,
        'opts.removeExtraIssuesArg');
    ...
    var context = {
        ...
        extraIssues: [],
        addExtraIssuesArg: opts.addExtraIssuesArg || '',
        removeExtraIssuesArg: opts.removeExtraIssuesArg || '',
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function printExtraIssues(arg, next) {
            console.log('Extra issues: %s',
                arg.extraIssues.map(function (issue) {
                    return issue.issueId; }
                ).join(','));
            next();
        },
