...
function stepQueryCRs(arg, cb) {
    ...
    common.forkExecWaitAndLog({
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (!cr_res.hasOwnProperty('url')) {
                continue;
            } else {
                arg.grrReviews.push(cr_res);
            }
        }
        cb();
    });
...
function stepReviewCRs(arg, cb) {
    ...
    vasync.forEachPipeline({
        ...
        func: function processReview(argv, next) {
            ...
            if (!arg.diffArg) {
                ...
                request.get(patch_url, {
                    ...
                    if (err) {
                        next(new VError(err, 'could not retrieve JIRA patch %s',
                            patch_url));
                        return;
                    } else if (res.statusCode === 404) {
                        next(new VError(err, 'no such JIRA patch %s',
                           patch_url));
                        return;
                    } else if (res.statusCode !== 200) {
                        next(new VError(
                            'unexpected JIRA response status for patch %s: %s',
                    ...
                    try {
                        ...
                        if (arg.dryRun) {
                            console.log('Dry run, will not prompt for review.');
                            return next();
                        }
                                ...
                                if (result.answer == 'y') {
                                    ...
                                    common.forkExecWaitAndLog({
                                        argv: ['ssh', '-q',
                                            ...
                                            arg.gerritUsername + '@cr.joyent.us',
                                            'gerrit', 'review',
                                            '--project', cr_res.project,
                                            '--label', arg.reviewArg + '=+1',
                                            cr_res.currentPatchSet.revision
                                        ],
                                        log: arg.log
        ...
        if (pipelineErr) {
            arg.log.trace({err: pipelineErr}, 'pipelineErr');
            cb(pipelineErr);
        }
    });
}
