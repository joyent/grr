...
/*
 * ssh cr gerrit query --patch-sets --format=JSON $crNumber
 *      | head -1 | json patchSets.-1.number
 */
function setGetCurrentPatchSet(arg, cb) {
    assert.string(arg.branchConfigCr, 'arg.branchConfigCr');

    common.forkExecWaitAndLog({
        argv: ['ssh', '-q', '-o', 'StrictHostKeyChecking=no',
            '-o', 'UserKnownHostsFile=/dev/null', '-p', '29418',
            'cr.joyent.us', 'gerrit', 'query', '--current-patch-set',
        ...
        arg.currentPatchSet = hit.currentPatchSet;
        arg.log.trace({cr: arg.branchConfigCr,
            currentPatchSet: arg.currentPatchSet}, 'currentPatchSet');
        cb();
    });
}
...
function stepCreateUpdateCr(arg, cb) {
    ...
    var commitMsg = arg.issueName + ' ' + arg.issueTitle;
    ...
    vasync.pipeline({arg: arg, funcs: [
         ...
         * code block.
         */
        function addCommitsToCr(_, next) {
            ...
            var gerritCmd = 'review -m "' + comment + '" '
                + arg.branchConfigCr + ',' + arg.currentPatchSet.number;

            common.forkExecWaitAndLog({
                argv: ['ssh', '-q', '-o', 'StrictHostKeyChecking=no',
                    '-o', 'UserKnownHostsFile=/dev/null',
                    '-p', '29418', 'cr.joyent.us', 'gerrit', gerritCmd],
                log: arg.log
            }, next);
        },

        function printResult(_, next) {
            console.log('CR %s: %s patchset %s <https://cr.joyent.us/%s>',
                (createdNewCr ? 'created' : 'updated'),
                arg.branchConfigCr,
                arg.currentPatchSet.number,
                arg.branchConfigCr);
            next();
        }
    ]}, function (err) {
