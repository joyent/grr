...
// XXX timf: use my private instance instead
// var GERRIT_HOST = 'kgerrit';
...
function stepValidateIssueArg(arg, cb) {
    ...
    if (!arg.issueId) {
        // https://jira.joyent.us/browse/FOO-123
        match = /^https:\/\/jira\.joyent\.us\/browse\/([A-Z]+-\d+)$/
            .exec(arg.issueArg);
        if (match) {
            arg.issueType = 'jira';
...
function stepGetRepoName(arg, cb) {
    ...
    var patterns = [
        ...
        new RegExp('github\.com\/(.+?)(\.git)?$'),
        // XXX timf, only needed for my private gerrit instance which doesn't
        // have .git suffices on the repos. Remove before putback.
        new RegExp('.*\/(.+)?(\.git)$')
    ];
            ...
            if (arg.branchConfigCr) {
                ...
                common.forkExecWaitAndPrint({
                    ...
                    if (err) {
                        ...
                        // XXX timf: hack, my private instance isn't using https
                        // remove before putback.
                        // var re = /^remote:   http:\/\/kgerrit:8080\/c\/(.*)\/(\d+) /m;
                        var match = re.exec(res.stderr);
                        if (!match) {
...
function stepListCRs(arg, cb) {
    ...
    common.forkExecWaitAndLog({
        ...
        for (var i=0; i < lines.length ; i++) {
            ...
            created.setUTCSeconds(res.createdOn);
            var subject = res.subject;
            var SUBJ_LIMIT = 80;
            if (subject.length > SUBJ_LIMIT) {
                subject = subject.slice(0, SUBJ_LIMIT) + '...';
            }
            // XXX timf: my private gerrit instance doesn't have emails set for
            // all users. Remove before putback.
            if (res.owner['email'] == null) {
                res.owner['email'] = '<unknown>';
            }
            console.log(
                extsprintf(
                    fmt, created.toISOString(), res.url, res.owner['email'],
                    subject));
        }
    });
}
...
/*
 * List open code reviews for *any* repository that match our issue along with
 * diffs showing the latest revision. Eventually, ask the user whether they
 * want to apply a given CR +1 type to each review, respecting any --dry-run
 * option passed.  XXX for now, we only list matching reviews and optionally
 * their diffs.
 */
function stepQueryCRs(arg, cb) {
    ...
    // should probably futher verify that this is one of our known
    // review type values, 'cr' or 'ia'
    ...
    common.forkExecWaitAndLog({
        ...
        console.log(extsprintf(fmt, 'CREATED', 'PROJECT', 'URL', 'AUTHOR', 'CR', 'IA', 'SUBJECT'));
        for (var i=0; i < lines.length ; i++) {
            var element = lines[i];
            var cr_res = JSON.parse(element);
            ...
            var created = new Date(0);
            created.setUTCSeconds(cr_res.createdOn);
            var subject = cr_res.subject;
            var SUBJ_LIMIT = 80;
            var cr = 'no';
            var ia = 'no';
            if (subject.length > SUBJ_LIMIT) {
            ...
            var approvals = cr_res.currentPatchSet.approvals;
            ...
            var review_summary = extsprintf(
                fmt, created.toISOString(), cr_res.project, cr_res.url,
                cr_res.owner['email'], cr, ia, subject);
            if (!arg.diffArg) {
                // emit a summary of the status of the review
                console.log(review_summary);
            } else {
                // I'd like to list diffs after each status line, so perhaps
                // the answer here is to merely gather the status data for now,
                // then have another pipeline stage to gather the diff data,
                // and a final stage to do all the printing?
                var revision = cr_res.currentPatchSet.revision;
                var url = mod_url.parse(cr_res.url);
                var gerrit_url = url.protocol + '//' + url.host;
                var patch_url = gerrit_url + '/changes/' + cr_res.number +
                ...
                request.get(patch_url, {
                    ...
                    if (err) {
                        cb(new VError(err, 'could not retrieve JIRA patch %s',
                            patch_url));
                        return;
                    } else if (res.statusCode === 404) {
                        cb(new VError(err, 'no such JIRA patch %s', patch_url));
                        return;
                    } else if (res.statusCode !== 200) {
                        cb(new VError('unexpected JIRA response status for patch %s: %s',
            ...
            // XXX timf The plan at this point, is that we'd now prompt the user
            // with a "Do you want to apply this review? (y/n)" question, after
            // which we'd proceed to apply the +1 CR or IA. We'd include a
            // --force or --yes option to assume 'y', or honor the --dry-run/-n
            // option to do nothing. My async js-fu is starting to hurt here,
            // because I can't tell whether all of that that should be done as
            // separate pipeline steps, passing data between stages.
            // Either way, the notion of nesting this into further function
            // calls to do all that here feels really messy :-(  )
...
function grrUpdateOrCreate(opts, cb) {
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function printResult(arg, next) {
            ...
            case 'none':
                if (arg.branchConfigCr) {
                    console.log('CR unchanged: %s <https://%s/%s>',
                        arg.branchConfigCr, GERRIT_HOST, arg.branchConfigCr);
                } else {
