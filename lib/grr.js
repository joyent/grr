...
function stepRemoveExtraIssues(arg, cb) {
    assert.object(arg.log, 'arg.log');
    assert.string(arg.removeExtraIssuesArg, 'arg.removeExtraIssuesArg');
    assert.array(arg.extraIssues, 'arg.extraIssues');
    assert.func(cb, 'cb');

    var remove = [];

    fsplit(arg.removeExtraIssuesArg, ',').forEach(function (item, _) {
...
function stepParseExtraIssues(arg, cb) {
    assert.object(arg.log, 'arg.log');
    assert.string(arg.addExtraIssuesArg, 'arg.addExtraIssuesArg');
    assert.string(arg.issueId, 'arg.issueId');
    assert.array(arg.extraIssues, 'arg.extraIssues');
    assert.func(cb, 'cb');

    fsplit(arg.addExtraIssuesArg, ',').forEach(function (item, _) {
    ...
    vasync.forEachParallel({
        inputs: arg.extraIssues, func: function (issue, next) {
            parseIssueArg(issue, arg.repoName, issue.issueId, function (err) {
                ...
                if (seen_already.hasOwnProperty(issue.issueId)) {
                    next(new VError('duplicate extra issue ' + issue.issueId));
                    return;
                }

                seen_already[issue.issueId] = true;
                ...
                getIssueTitle(arg, issue, function (err2, title) {
                    ...
                    arg.log.trace({extraIssue: issue});
                    next();
                });
            });
        }
