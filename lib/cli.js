...
var options = [
    ...
    },
    {
        names: ['verbose', 'v'],
        type: 'bool',
        help: 'Verbose debugging output. Use `grr -v ... 2>&1 | bunyan` for '
            + 'a rendered view.'
    },
    {
...
function usage() {
    ...
    console.log([
        ...
        '',
        'Usage:',
        '    grr [<options>] [<issue> [<parenthetical>]]',
        '    grr --delete [<issue>]',
        '',
        'Options:',
        help,
...
function main(argv) {
    ...
    if (opts._args.length) {
        issue = opts._args.shift();
    }
    if (opts._args.length && !opts['delete']) {
        parenthetical = opts._args.shift();
    }
    if (opts._args.length) {
