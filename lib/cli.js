...
var options = [
    ...
    },
    {
        names: ['dry-run', 'n'],
        type: 'bool',
        help: 'Go through the motions without actually committing, pushing or '
            + 'reviewing.',
        'default': false
    }
];
...
function usage() {
    ...
    console.log([
        ...
        '',
        'Usage:',
        '    grr    [<issue> [<extra-issues>]]',
        '                          # start or update a CR',
        '    grr -D                # (D)elete "grr-*" feature branch',
        '    grr -L                # (L)ist outstanding CRs for this repo',
        '    grr -R <ia|cr> [-d] [-n] <issue>',
        '                          # (R)eview CRs matching grant this issue',
        '',
        'Options:',
        help,
        '',
        'Where <issue> is a JIRA issue (the PROJ-123 name or the full URL) or',
        'a GitHub issue (the number, account/repo#num, or full URL).',
        '',
        'Examples:',
        '    Working on changes:',
        '',
        '    grr TOOLS-123    # Start or update a branch and CR for this issue',
        //'    grr --commit TOOLS-123    # ... also commit uncommitted changes',
        ...
        '    grr -D           # All done. Delete the branch and switch',
        '                     # back to master',
        '    grr -L           # Show reviews in progress for this repository,',
        '                     # useful to see whether your changes might clash',
        '                     # with other work.',
        '',
        '    Reviewing changes:',
        '',
        '    grr -R cr TOOLS-124       # show review status for TOOLS-124',
        '    grr -R cr -d -n TOOLS-124 # show diffs for TOOLS-124',
        '                              # but only do a dry-run review.',
        '    grr -R ia -d TOOLS-124    # show diffs for TOOLS-124,',
        '                              # and prompt for IA approval for each.'
    ].join('\n'));
    /* END JSSTYLED */
}
...
function main(argv) {
    ...
    if ((opts._args.length > 0 || opts.update || opts.add_issues ||
        opts.remove_issues) && !(opts.list || opts.review))
    {
    ...
    if (opts.delete) {
        if (actions.length) {
            fatal('cannot delete (-D) when reviewing (-R), listing (-L), or '
    ...
    if (opts.list) {
        if (actions.length) {
            fatal('cannot list (-L) when deleting (-D), reviewing (-R) or '
                + 'creating/updating a CR');
        }
        actions.push('list');
    }
    ...
    if (opts.review) {
        if (actions.length) {
            fatal('cannot review (-R) when deleting (-D), listing (-L) or '
                + 'creating/updating a CR');
        }
        if (!VALID_REVIEWS.hasOwnProperty(opts.review)) {
            fatal('unknown review type ' + opts.review);
        }
        actions.push('review');
        if (opts._args.length === 1) {
            issueArg = opts._args.shift();
        } else {
            fatal('single issue required when using -R');
        }
    }
