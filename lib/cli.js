...
function usage() {
    ...
    console.log([
        ...
        '    grr -R <ia|cr>',
        '        [-d] [-n]',
        '        <issue>              # grant +1s for reviews matching this',
        '                             # issue',
        '',
        'Options:',
        help,
    ...
    '    grr -D           # All done. Del the branch and switch back to master',
        '',
    '    grr -R cr TOOLS-124       # show status of reviews matching this issue',
    '    grr -R cr -d -n TOOLS-124 # show diffs of changes matching this issue',
    '                              # but only do a dry-run review.',
    '    grr -R ia -d TOOLS-124    # show diffs of changes matching this issue',
    '                              # and prompt for y/n IA approval for each.'
    ].join('\n'));
    /* END JSSTYLED */
}
...
function main(argv) {
    ...
    if (opts.review) {
        ...
        if (!VALID_REVIEWS.hasOwnProperty(opts['review'])) {
            fatal('grr: unknown review type ' + opts['review']);
        }
        actions.push('review');
        if (opts._args.length > 0) {
            issueArg = opts._args.shift();
